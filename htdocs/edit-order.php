...
if ($order_info['is_bid']) {
	$buy_amount1 = (!empty($_REQUEST['buy_amount'])) ? String::currencyInput($_REQUEST['buy_amount']) : $order_info['btc'];
	$buy_market_price1 = (!empty($_REQUEST['buy_market_price'])) ? $_REQUEST['buy_market_price'] : ($order_info['market_price'] == 'Y');
	$buy_price1 = (!empty($_REQUEST['buy_price'])) ? String::currencyInput($_REQUEST['buy_price']) : (($order_info['btc_price'] > 0) ? $order_info['btc_price'] : ($buy_market_price1 ? $current_ask : 0));
	$buy_stop_price1 = (!empty($_REQUEST['buy_stop_price'])) ? String::currencyInput($_REQUEST['buy_stop_price']) : $order_info['stop_price'];
	...
	$sell_amount1 = (!empty($_REQUEST['sell_amount'])) ? String::currencyInput($_REQUEST['sell_amount']) : $order_info['btc'];
	$sell_market_price1 = (!empty($_REQUEST['sell_market_price'])) ? $_REQUEST['sell_market_price'] : ($order_info['market_price'] == 'Y');
	$sell_price1 = (!empty($_REQUEST['sell_price'])) ? String::currencyInput($_REQUEST['sell_price']) : (($order_info['btc_price'] > 0) ? $order_info['btc_price'] : ($buy_market_price1 ? $current_bid : 0));
	$sell_stop_price1 = (!empty($_REQUEST['sell_stop_price'])) ? String::currencyInput($_REQUEST['sell_stop_price']) : $order_info['stop_price'];
